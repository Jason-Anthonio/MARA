...
// ==========================================
enum SequenceState { IDLE, START_B, WAIT_B, START_C, WAIT_C, START_D, WAIT_D, START_A, WAIT_A };
SequenceState seqState = IDLE;

float pendingTargetA = 0.0; bool moveRequestedA = false;
float pendingTargetB = 0.0; bool moveRequestedB = false;
float pendingTargetC = 0.0; bool moveRequestedC = false;
float pendingTargetD = 0.0; bool moveRequestedD = false;
...
void setup() {
  ...
  Serial.println("Quad Motor PID System Ready (Sequential Mode).");
  Serial.println("Sequence: Base(B) -> Wrist(C) -> Elbow(D) -> Shoulder(A)");
...
void loop() {
  ...
  // =========================================================
  // --- SEQUENCE STATE MACHINE (B -> C -> D -> A) ---
  // =========================================================
  // The state machine evaluates freely on every loop iteration
  switch (seqState) {
    case IDLE:
      // Waiting for readSerialTarget() to initiate START_B
      break;

    case START_B:
      if (moveRequestedB) {
        finalTargetB = pendingTargetB;
        targetReachedB = false;
        moveRequestedB = false;
        seqState = WAIT_B;
      } else {
        seqState = START_C; 
      }
    ...
    case WAIT_B:
      if (targetReachedB) seqState = START_C;
      break;

    case START_C:
      if (moveRequestedC) {
        finalTargetC = pendingTargetC;
        targetReachedC = false;
        moveRequestedC = false;
        seqState = WAIT_C;
      } else {
        seqState = START_D; 
      }
    ...
    case WAIT_C:
      if (targetReachedC) seqState = START_D;
      break;

    case START_D:
      if (moveRequestedD) {
        finalTargetD = pendingTargetD;
        targetReachedD = false;
        moveRequestedD = false;
        seqState = WAIT_D;
      } else {
        seqState = START_A; 
      }
    ...
    case WAIT_D:
      if (targetReachedD) seqState = START_A;
      break;

    case START_A:
      if (moveRequestedA) {
        finalTargetA = pendingTargetA;
        targetReachedA = false;
        moveRequestedA = false;
        seqState = WAIT_A;
      } else {
        seqState = IDLE; 
      }
    ...
    case WAIT_A:
      if (targetReachedA) seqState = IDLE;
      break;
  }

  // =========================================================
  // --- PID TIMER BRACKET (Replaced the 'return' trap) ---
  // =========================================================
  unsigned long now = micros();
  if (now - prevPidMicros >= PID_SAMPLE_US) {
  ...
  } // <-- End of the 10ms PID block

  // =========================================================
  // --- DEBUG OUTPUT ---
  // =========================================================
  // Safely outside the PID timing block. Will trigger exactly when PRINT_MS is reached.
  if (millis() - prevPrintMillis >= PRINT_MS) {
    if (!targetReachedA || !targetReachedB || !targetReachedC || !targetReachedD) {
      Serial.print("A[P:"); Serial.print(counterA); // Direct counter read for debugging
      Serial.print(" T:"); Serial.print(finalTargetA); Serial.print("] | ");
      Serial.print("B[P:"); Serial.print(counterB); 
      Serial.print(" T:"); Serial.print(finalTargetB); Serial.print("] | ");
      Serial.print("C[P:"); Serial.print(counterC); 
      Serial.print(" T:"); Serial.print(finalTargetC); Serial.print("] | ");
      Serial.print("D[P:"); Serial.print(counterD); 
    ...
    if (inChar == '\n') {
      if (inputString.length() > 0) {
        ...
        if (aIndex != -1) {
          ...
          pendingTargetA = targetAngleA * (PPR_A / 360.0);
          moveRequestedA = true;
        }
        
        // --- Parse JGA25 (B) --- 
        // Kept original structure (unbounded string parsing) as requested
        if (bIndex != -1) {
          targetAngleB = inputString.substring(bIndex + 1).toInt();
          pendingTargetB = targetAngleB * (PPR_B / 360.0);
          moveRequestedB = true;
        }
        ...
        if (cIndex != -1) {
          ...
          pendingTargetC = targetAngleC * (PPR_C / 360.0);
          moveRequestedC = true;
        }
        ...
        if (dIndex != -1) {
          ...
          pendingTargetD = targetAngleD * (PPR_D / 360.0);
          moveRequestedD = true;
        }

        // --- KICKSTART SEQUENCE ---
        if (moveRequestedA || moveRequestedB || moveRequestedC || moveRequestedD) {
          if (seqState == IDLE) {
            seqState = START_B; 
          }
        }
